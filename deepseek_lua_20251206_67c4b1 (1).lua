...
local function findNearestVisibleEnemy()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer and player.Character and isEnemy(player) then
            ...
            if humanoid and humanoid.Health > 0 and rootPart and localPlayer.Character then
                local localRoot = localPlayer.Character:FindFirstChild("HumanoidRootPart")
                if localRoot then
                    local distance = (rootPart.Position - localRoot.Position).Magnitude
                    
                    if distance < shortestDistance and distance < 100 then
                        if canSeeTarget(rootPart.Position) then
...
local function calculateAimPosition(targetRoot, localRoot)
    ...
    local predictedPos = targetPos + (targetRoot.Velocity * travelTime)
    
    local heightDifference = targetPos.Y - localPos.Y
    local verticalOffset = math.clamp(heightDifference * 0.1, -1, 2)
    
    return Vector3.new(predictedPos.X, predictedPos.Y + verticalOffset, predictedPos.Z)
end

local function aimAtTarget(targetEnemy)
    if not targetEnemy or not targetEnemy.Character then return false end
    
    local targetRoot = targetEnemy.Character:FindFirstChild("HumanoidRootPart")
    local localRoot = localPlayer.Character:FindFirstChild("HumanoidRootPart")
    
    if not targetRoot or not localRoot then return false end
    
    local aimPosition = calculateAimPosition(targetRoot, localRoot)
    local direction = (aimPosition - localRoot.Position).Unit
    
    local lookCFrame = CFrame.lookAt(localRoot.Position, Vector3.new(aimPosition.X, localRoot.Position.Y, aimPosition.Z))
    localRoot.CFrame = lookCFrame
    
    local currentTool = localPlayer.Character:FindFirstChildOfClass("Tool")
    if currentTool then
        local screenPoint = workspace.CurrentCamera:WorldToScreenPoint(aimPosition)
        local mouse = localPlayer:GetMouse()
        mouse.TargetFilter = targetEnemy.Character
    end
...
local function updateAimbot()
    ...
    if Aimbot.Enabled and localPlayer.Character then
        Aimbot.Connection = RunService.RenderStepped:Connect(function()
            ...
            if currentTime - Aimbot.LastAimTime > 0.033 then
                local targetEnemy, distance = findNearestVisibleEnemy()
                if targetEnemy then
                    aimAtTarget(targetEnemy)
                end
                Aimbot.LastAimTime = currentTime
            end
        end)
    end
end
...
-- */  Aimbot Tab  /* --
do
    local AimbotTab = ElementsSection:Tab({
        Title = "Aimbot",
        Icon = "target"
    })
    ...
    AimbotTab:Toggle({
        Title = "Enable Aimbot",
        Desc = "Toggle auto aim feature on/off",
        Flag = "AimbotEnabled",
        Default = false,
    ...
    AimbotTab:Button({
        Title = "Test Aimbot",
        Desc = "Find nearest enemy and aim",
        Icon = "search",
        Callback = function()
            local targetEnemy, distance = findNearestVisibleEnemy()
            if targetEnemy then
                ...
                WindUI:Notify({
                    Title = "Aimbot Test",
                    Content = "Target found: " .. targetEnemy.Name .. " (" .. math.floor(distance) .. " studs)",
                    Icon = "check"
                })
            else
                WindUI:Notify({
                    Title = "Aimbot Test",
                    Content = "No enemies found",
                    Icon = "x"
                })
            end
        end
    ...
    AimbotTab:Toggle({
        Title = "Team Check",
        Desc = "Only target enemy team members",
        Default = true,
        Callback = function(state)
            -- Team check is already integrated in isEnemy function
            WindUI:Notify({
                Title = "Team Check",
